/**************************************************************************************************************************************************************
 Auteur : Marie-Pier Morin & Fred
 Date de création : 8 novembre 2024
 Modification : Marie-Pier Morin
 Date de modification : 11 décembre 2024
 Note : Programme contenant tout ce qu'il faut pour faire fonctionner le véhicule lors
 du projet de fin de session.
 "J'aurais aimer avoir le temps de diviser mon programme en plusieurs fichiers .h et .c
 Mais je voulais priorisé le fonctionnement rapide de mon programme avant de tout diviser
 et finalement je n'ai pas eu le temps malheureusement."
 *************************************************************************************************************************************************************/
...
#define FONCTION 0x11
#define DEPLACEMENT 0x12
#define ARRET 0x13
#define ERREUR 0x14
#define ATTENDCOMMANDE 0x15

int x;
int err;
int Position;
static int compteur = 0x00;
...
void loop() {
  ...
  switch (etatVehicule) {
    ...
    case EN_ATTENTE:
    Arreter();
      if (ModeSuiveurOrange || ModeSuiveurMetal) { 
...
void DemiTour() {
  ...
  analogWrite(DAV2, 100);  // Contrôle de vitesse via PWM
  analogWrite(GAR1, 105);  // Contrôle de vitesse via PWM
  analogWrite(GAR2, LOW);  
  analogWrite(DAR1, LOW);  
  analogWrite(DAR2, 100);  // Contrôle de vitesse via PWM
  if (Position == 0) {
    Arreter();
...
/**************************************************************************************************
 ...
 Date de création : 8 novembre 2024
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction pour le mode suiveur de ligne.
 ***************************************************************************************************/
void ModeSuiveur() {
  LectureI2C();
  Position = checkValue();
...
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction pour avancer tout droit
 ***************************************************************************************************/
void allerToutDroit() {  
...
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction pour tourner à gauche
 ***************************************************************************************************/
void tournerGauche() {  
  analogWrite(GAV1, LOW);
...
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction pour tourner à gauche beaucoup
 ***************************************************************************************************/
void tournerBeaucoupGauche() {  
  analogWrite(GAV1, LOW);
  analogWrite(GAV2, 100);  // Contrôle de vitesse via PWM
  analogWrite(DAV1, LOW);
  analogWrite(DAV2, 95);   // Contrôle de vitesse via PWM
...
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction pour tourner à droite
 ***************************************************************************************************/
void tournerDroite() {  
  analogWrite(GAV1, LOW);
  analogWrite(GAV2, 100);  // Contrôle de vitesse via PWM
...
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction pour tourner à droite beaucoup
 ***************************************************************************************************/
void tournerBeaucoupDroite() {  
  analogWrite(GAV1, LOW);
  analogWrite(GAV2, 100);  // Contrôle de vitesse via PWM
  analogWrite(DAV1, LOW);
  analogWrite(DAV2, 95);  // Contrôle de vitesse via PWM
...
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction pour tourner à droite beaucoup plus
 ***************************************************************************************************/
void tournerBeaucoupDroitePLUS() {  
  analogWrite(GAV1, LOW);
  analogWrite(GAV2, 100);  // Contrôle de vitesse via PWM
  analogWrite(DAV1, 100);  // Contrôle de vitesse via PWM
  analogWrite(DAV2, LOW);
...
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction pour tourner à gauche beaucoup plus
 ***************************************************************************************************/
void tournerBeaucoupGauchePLUS() {  
  analogWrite(GAV1, 110);  // Contrôle de vitesse via PWM
  analogWrite(GAV2, LOW);
  analogWrite(DAV1, LOW);
...
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction pour gérer la détection d'une ligne transversale
 ***************************************************************************************************/
...
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction de reception des informations qui passe pars la passerelle.
 ***************************************************************************************************/
void udpReceiveData() {
  uint8_t incomingPacket[4]; // Tableau pour stocker exactement 4 octets de données
  memset(incomingPacket, 0, sizeof(incomingPacket));

  int packetSize = udp.parsePacket(); // Vérifie si des données arrivent
  if (packetSize == 4) { // S'assure que le paquet contient exactement 4 octets
    udp.read(incomingPacket, sizeof(incomingPacket)); // Lit les 4 octets

    // Affichage des données reçues pour débogage
    // Serial.println("Données reçues (hex) :");
    // for (int i = 0; i < 4; i++) {
    ...
    // 1er octet : Commande principale
    if (incomingPacket[0] == 0x01) { // Mode Abort
      ModeAbort = true;
      EnFonction = false;
      Serial.println("Commande reçue : Abort");
    }

    // 2ème octet : État de fonctionnement
    if (incomingPacket[1] == 0x41) { // Mode Abort
      ModeAbort = true;
      EnFonction = false;
      Serial.println("Commande reçue : Abort");
    } else if (incomingPacket[1] == 0x40) { // Mode EnFonction
      ModeAbort = false;
      EnFonction = true;
      etat = FONCTION; // Change l'état
      Serial.println("Commande reçue : EnFonction");
    }

    // 3ème octet : Type de bloc
    if (incomingPacket[2] == 0x61) {
      Serial.println("Commande reçue : BlocNoir (Aucune action)");
    } else if ((incomingPacket[2] == 0x62)&&(EnFonction == true)) {
      ModeSuiveurOrange = true;
      Serial.println("Commande reçue : BlocOrange");
    } else if ((incomingPacket[2] == 0x63)&&(EnFonction == true)) {
      ModeSuiveurMetal = true;
      Serial.println("Commande reçue : BlocMetal");
    }

    // 4ème octet : Action du véhicule
    if (incomingPacket[3] == 0x81) {
      Vehicule = true; // Charger
      Serial.println("Commande reçue : Charger");
    } else if (incomingPacket[3] == 0x82) {
      Vehicule = false; // Décharger
      Serial.println("Commande reçue : Décharger");
    } else if (incomingPacket[3] == 0x83) {
      Poids = true; // Peser
      Serial.println("Commande reçue : PoidsPeser");
    }

  } else if (packetSize > 0) {
    Serial.println("Paquet reçu mais taille incorrecte (doit être de 4 octets).");
  } else {
    // Aucun paquet reçu
  }
}
/**************************************************************************************************
 ...
 Modification : Marie-Pier Morin
 Date de modification : 11 decembre 2024
 Note : Fonction de transmission des informations qui vont vers la passerelle.
 ***************************************************************************************************/
void udpSend() {
