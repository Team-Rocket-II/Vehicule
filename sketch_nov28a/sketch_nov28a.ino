...
void ModeSuiveur() {
  ...
  // Changer la variable Position dans la loop pour toujours vérifier ou on est rendu.
  Position = checkValue();
  if (Position == -3 || Position == -2 || Position == -1) {  // Capteur de ligne à droite actif
    tournerDroite();
  } else if (Position == 0) {  // Capteur de ligne au milieu actif
    allerToutDroit();
  } else if (Position == 1 || Position == 2 || Position == 3) {  // Capteur de ligne à gauche actif
    tournerGauche();
...
void tournerGauche() {
  analogWrite(GAV1, vitesseMoteurTourne);
  analogWrite(GAV2, LOW);  // Contrôle de vitesse via PWM
  analogWrite(DAV1, LOW);  // Contrôle de vitesse via PWM
  analogWrite(DAV2, vitesseMoteurTourne);
  analogWrite(GAR1, vitesseMoteurTourne);
  analogWrite(GAR2, LOW);  // Contrôle de vitesse via PWM
  analogWrite(DAR1, LOW);  // Contrôle de vitesse via PWM
  analogWrite(DAR2, vitesseMoteurTourne);
...
void tournerDroite() {
  analogWrite(GAV1, LOW);  // Contrôle de vitesse via PWM
  analogWrite(GAV2, vitesseMoteurTourne);
  analogWrite(DAV1, vitesseMoteurTourne);
  ...
  analogWrite(DAR1, vitesseMoteurTourne);
